...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<Account?> GetUserByIdAsync(string userId, Account account)
        {
            var user = await _userManager.FindByIdAsync(userId);

            if ((account.Organization != null) && (!((user != null) && (user.Organization == account.Organization))))
            {
                return null;
            }
        ...
        public async Task<Account?> GetUserByUserNameAsync(string userName, Account account)
        {
            var user = await _userManager.FindByNameAsync(userName);

            if (!((user != null) && (user.Organization == account.Organization))) { return null; }
        ...
        public async Task<(Account User, string[] Roles)?> GetUserAndRolesAsync(string userId, Account account)
        {
            Account? user = null;
            ...
            if (_context.Users != null)
            {
                ...
                {
                    user = await _context.Users.Where(x => x.Organization == account.Organization)
                        .Include(u => u.Roles)
                        .Where(u => u.Id == userId)
                        .SingleOrDefaultAsync();
                }
        ...
        public async Task<List<(Account User, string[] Roles)>> GetUsersAndRolesAsync(int page, int pageSize, Account account)
        {
            IQueryable<Account> usersQuery = null;
            ...
            {
                usersQuery = _context.Users.Where(x => x.Organization == account.Organization)
                    .Include(u => u.Roles)
                    .OrderBy(u => u.UserName);
            }
        ...
        {
            var tmpAccount = _context.Users.Where(u => u.Id.Equals(user.Id)).Include(u => u.ToDoItems).FirstOrDefault();
            if (tmpAccount?.ToDoItems.Count <= 0)
            {
            ...
            {
                return (false, new string[]{"User has assigned ToDo items. Unable to delete"});
            }
        }
